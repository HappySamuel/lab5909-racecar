  ...
  use_servo_cmd_(true), publish_tf_(false), x_(0.0), y_(0.0), yaw_(0.0)
{
  ...
  if (!getRequiredParam(nh, "speed_to_erpm_offset", speed_to_erpm_offset_))
    return;
  if (!getRequiredParam(nh, "current_speed_gain", current_speed_gain_))
    return;	//SAMUEL - added for adjusting current speed gain
  if (use_servo_cmd_) {
  ...
  odom_pub_ = nh.advertise<nav_msgs::Odometry>("vesc/odom", 10);   //SAMUEL - edited extra using "vesc"
...
void VescToOdom::vescStateCallback(const vesc_msgs::VescStateStamped::ConstPtr& state)
{
  ...
  double current_speed = current_speed_gain_*( state->state.speed - speed_to_erpm_offset_ ) / speed_to_erpm_gain_; //SAMUEL - added "current_speed_gain_"
